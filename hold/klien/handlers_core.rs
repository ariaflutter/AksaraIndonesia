...
async fn is_authorized(&self) -> Result<bool, StatusCode> {
    match self.claims.role {
        ...
        crate::types::UserRole::AdminBapas => {
            ...
            if let Some(user_bapas_id) = self.claims.bapas_id {
...
pub async fn create_klien(
    ...
    match claims.role {
        ...
        crate::types::UserRole::AdminBapas => {
            target_bapas_id = claims.bapas_id.ok_or(StatusCode::FORBIDDEN)?;
            target_pk_id = payload.pk_id.ok_or(StatusCode::BAD_REQUEST)?;
        }
...
pub async fn get_all_klien(
    ...
    match claims.role {
        ...
        crate::types::UserRole::AdminBapas => {
            // AdminBapas sees all clients within their bapas_id.
            if let Some(bapas_id) = claims.bapas_id {
                query_builder.push(" WHERE bapas_id = ");
                query_builder.push_bind(bapas_id);
            } else {
